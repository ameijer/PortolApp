...
public class UserUpdateThread extends Thread {
    ...
    public UserUpdateThread(User toupdate) {
        super();
    ...
    public void run() {
        ...
        try {
            Log.i(TAG, "JSON being sent: " + mapper.writeValueAsString(toUpdate));
        } catch (JsonProcessingException e3) {
            // TODO Auto-generated catch block
            e3.printStackTrace();
        }

        HttpPost httpPost = new HttpPost(server + ":" + port + "/api/v0/user/update");
        try {


            httpPost.setEntity(new StringEntity(mapper.writeValueAsString(toUpdate)));
